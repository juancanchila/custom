...
class Liquidadorepa_FormEn extends FormBase
{
  /**
   * The session object.
   *
 ...
 //pagina 1
           ...
           $form['id_document'] = [
            ...
            '#default_value' => $this->session->get('session_liquidacion.id_document_e',''),
            ];
            ...
            $form['name'] = [
             ...
             '#default_value' => $this->session->get('session_liquidacion.name_e', ''),
            ];
            ...
            $form['dir_correspondencia'] = [
              ...
              '#default_value' => $this->session->get('session_liquidacion.dir_correspondencia_e', ''),
             ];
             $form['email'] = [
              '#type' => 'email',
             '#required' => TRUE,
              '#title' => 'Correo Electrónico',
              '#default_value' => $this->session->get('session_liquidacion.email_e', ''),
             ];
             ...
             $form['tfijo'] = [
              '#type' => 'textfield',
              '#title' => 'Teléfono fijo',
              '#default_value' => $this->session->get('session_liquidacion.tfijo_e', ''),
              ];
              ...
              $form['tmovil'] = [
                ...
                '#default_value' => $this->session->get('session_liquidacion.tmovil_e', ''),
               ];

               $form['estrato'] = array(
                '#title' => t('Estrato'),
                '#default_value' => $this->session->get('session_liquidacion.estrato_e', ''),
                '#type' => 'select',
               ...
               $form['condicion'] = array(
                  ...
                  '#default_value' => $this->session->get('session_liquidacion.condicion_e', ''),
                  '#type' => 'select',
...
//validate all form
 ...
 $valor_tarifa=$this->session->get('session_liquidacion.valor_tarifa_en', '');
 $valor_liquidacion  = $this->session->get('session_liquidacion.valorLiquidacion_en', '');
 $valor_evento = $this->session->get('session_liquidacion.valor_evento_e', '');
 $valor_evento = number_format( $valor_evento, 2, ',', '.');


    $codigo_liquidacion =$this->session->get('session_liquidacion.id_document_e', '');
    $name_contrib = $this->session->get('session_liquidacion.name_e', '');
    $barrio_liquidacion = $this->session->get('session_liquidacion.barrio_e', '');



    $valore =$valor_evento;
    $descripcion_evento = $this->session->get('session_liquidacion.description_evento_e', '');
    $direccion_evento = $this->session->get('session_liquidacion.direccion_evento_e', '');
    $tipo_solicitante = "Persona Natural";
    $id_contribuyente = $this->session->get('session_liquidacion.id_document_e', '');
    $numero_dias = $this->session->get('session_liquidacion.numero_dias_e', '');


    $dir_correspondecia_contrib = $this->session->get('session_liquidacion.dir_correspondencia_e', '');
    $email_cotrib = $this->session->get('session_liquidacion.email_e', '');
    $tfijo =$this->session->get('session_liquidacion.tfijo_e', '');
    $tmovil = $this->session->get('session_liquidacion.tmovil_e', '');
    $estrato = $this->session->get('session_liquidacion.estrato_e', '');
    $condicion = $this->session->get('session_liquidacion.condicion_e', '');
    $descripcion_evento = $this->session->get( 'session_liquidacion.description_evento_e', '');

        $f1 =$this->session->get('session_liquidacion.fecha_Inicial_e', '');
        $f2 =$this->session->get('session_liquidacion.fecha_Final_e', '');
        $file1 = $this->session->get('session_liquidacion.soportes1_e');
        $file2 = $this->session->get('session_liquidacion.soportes2_e');
        $file3 = $this->session->get('session_liquidacion.soportes3_e');
...
*/
 ...
 $valor_liquidacion =  2773866  *  $numero_dias ;
 $valor_liquidacion_r =  1904150  *  $numero_dias ;
  ...
  $valor_liquidacion =  3467564  *  $numero_dias  ;
 ...
 $valor_liquidacion = 4854844*  $numero_dias ;
 $valor_liquidacion_r = 3332600 *  $numero_dias;
    ...
    $valor_tarifa = 6242123;
  $valor_liquidacion = 6242123  *  $numero_dias ;
  $valor_liquidacion_r = 4284900 *  $numero_dias;
    ...
    $valor_tarifa =104039624;
 $valor_liquidacion = 104039624  *  $numero_dias ;
 $valor_liquidacion_r = 104039620 *  $numero_dias;
    ...
    $form_state->setErrorByName('fecha_Inicial', $this->t('La Cantidad de días no es correcta '. $error_dias));
           ...
           public function fapiExampleMultistepFormNextSubmit(array &$form, FormStateInterface $form_state)
           {
            ...
            'name' => $form_state->getValue('name'),
             ...
             ->setRebuild(TRUE);
                 $this->setSessionValue('session_liquidacion.id_document_e', $form_state->getValue('id_document'));
                 $this->setSessionValue('session_liquidacion.name_e', $form_state->getValue('name'));
                 $this->setSessionValue('session_liquidacion.dir_correspondencia_e', $form_state->getValue('dir_correspondencia'));
                 $this->setSessionValue('session_liquidacion.email_e', $form_state->getValue('email'));
                 $this->setSessionValue('session_liquidacion.estrato_e', $form_state->getValue('first_estrato'));
                 $this->setSessionValue('session_liquidacion.tmovil_e', $form_state->getValue('tmovil'));
                 $this->setSessionValue('session_liquidacion.tfijo_e', $form_state->getValue('tfijo'));
             }
 ...
 /**
  ...
  public function fapiExampleMultistepFormNextSubmit2(array &$form, FormStateInterface $form_state) {
    $form_state
      ...
      ->setRebuild(TRUE);
      $this->setSessionValue('session_liquidacion.description_evento_e', $form_state->getValue('descripcion_evento_e'));
      $this->setSessionValue('session_liquidacion.barrio_e', $form_state->getValue('barrio'));
      $this->setSessionValue('session_liquidacion.valor_evento_e', $form_state->getValue('valor_evento_e'));
      $this->setSessionValue('session_liquidacion.numero_dias_e', $form_state->getValue('numero_dias_e'));
      $this->setSessionValue('session_liquidacion.direccion_evento_e', $form_state->getValue('direccion_evento'));
  ...
  public function fapiExampleMultistepFormNextSubmit3(array &$form, FormStateInterface $form_state) {
    $form_state
      ->set('page_values4', [
        ...
        'soportes1_e' => $form_state->getValue('soportes1_e'),
      ...
      // to 2, the AJAX-rendered form will still show page 3.
      ->setRebuild(TRUE);
      $this->setSessionValue('session_liquidacion.soportes1_e', $form_state->getValue('soportes1'));
      $this->setSessionValue('session_liquidacion.soportes2_e', $form_state->getValue('soportes2'));
      $this->setSessionValue('session_liquidacion.soportes3_e', $form_state->getValue('soportes3'));
  }
  /**
  ...
  public function fapiExamplePageTwo(array &$form, FormStateInterface $form_state) {
    ...
    $form['description_evento'] = [
      '#type' => 'item',
      '#title' => $this->t('Detalles del Evento'),
      '#default_value' => $this->session->get('session_liquidacion.description_evento', ''),
    ];
    ...
    $form['barrio'] = [
      ...
      '#default_value' => $this->session->get('session_liquidacion.barrio_e', ''),
        ];
    ...
    '#title' => $this->t('Valor Inversión'),
      ...
      '#width' => '30%',
      '#align' => 'center',
      '#default_value' => $this->session->get('session_liquidacion.valor_evento_e', ''),
      '#required' => true,
      '#maxlength' =>10
  );
  ...
  $form['fecha_Final'] = array (
   ...
   '#title' => 'Ingresar Fecha de finalización del Evento',
    ...
    $form['numero_dias_e'] = array(
      '#type' => 'number',
      '#title' => 'Ingresar duración del evento en días',
      '#width' => '30%',
      '#align' => 'center',
      '#default_value' => $this->session->get('session_liquidacion.numero_dias_e', ''),
      '#required' => true,
      '#maxlength' => 3,
		 '#min' =>1,
  );
      ...
      $form['descripcion_evento_e'] = [
        '#type' => 'textfield',
        '#title' => 'Breve Descripción del Evento',
        '#default_value' => $this->session->get('session_liquidacion.descripcion_evento_e', ''),
       '#required' => TRUE,
      ];
      $form['direccion_evento'] = [
        '#type' => 'textfield',
        '#title' => 'Dirección donde se realizará el Evento',
        '#default_value' => $this->session->get('session_liquidacion.direccion_evento_e', ''),
      '#required' => TRUE,
      ];
    $form['back'] = [
      '#type' => 'submit',
  ...
  $form['list'] = [
    ...
    $form['soportes1'] = array(
      ...
      '#default_value' => $this->session->get('session_liquidacion.soportes1_e',''),
    );
    ...
    $form['soportes2'] = array(
      ...
      '#upload_validators' => $validators,
      '#upload_location' => 'public://my_files/privado',
      '#default_value' => $this->session->get('session_liquidacion.soportes2_e',''),
    );
    ...
    $form['soportes3'] = array(
      ...
      '#size' => 20,
      '#description' => 'Certificación , Cotización o Factura del valor invertido en evento a evaluar : Límite: 2MB./ PDF',
      '#upload_validators' => $validators,
      '#upload_location' => 'public://my_files/privado',
      '#default_value' => $this->session->get('session_liquidacion.soportes3_e',''),
    );
  ...
  public function fapiExamplePageFour(array &$form, FormStateInterface $form_state) {
    ...
    // $form_state->set('page_num', 4);
     ...
     $id= $this->session->get('session_liquidacion.id_document_e', '');
     $email=$this->session->get('session_liquidacion.email_e', '');
     $Cantidad=$this->session->get('session_liquidacion.valor_evento_e', '');
     $Cantidad = number_format($Cantidad, 2, ',', '.');
     $alert=$this->session->get('session_liquidacion.alert_e', '');
     $tarifa =  $this->session->get('session_liquidacion.valor_tarifa_en', '');
     $total_liquidacion = $this->session->get('session_liquidacion.valorLiquidacion_en', '');
      $form['s0010'] = [
       '#type' => 'markup',
       '#markup' =>   '
     <div class="row">
