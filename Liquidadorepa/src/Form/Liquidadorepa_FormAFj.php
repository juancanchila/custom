...
class Liquidadorepa_FormAFj extends FormBase
{
  /**
   * The session object.
   *
 ...
 /**
        ...
        return self::fapiExamplePageFour($form, $form_state);
           ...
           $form['s002'] = [
            ...
            $form['id_document'] = [
              ...
              '#required' => TRUE,
              '#title' => 'NIT',
              '#default_value' => $this->session->get('session_liquidacion.id_document_afj',''),
              ];
              ...
              $form['name'] = [
                ...
                '#default_value' => $this->session->get('session_liquidacion.name_afj', ''),
               ];
               $form['nombrelegal'] = [
                '#type' => 'textfield',
                '#size' => '60',
                ...
                '#description' => 'Razón Social de la entidad.',
                '#default_value' => $this->session->get('session_liquidacion.nombrelegal_afj', ''),
               ...
               $form['idlegal'] = [
                ...
                '#default_value' => $this->session->get('session_liquidacion.idlegal_afj', ''),
               ];
               $form['dir_correspondencia'] = [
                '#type' => 'textfield',
                '#required' => TRUE,
                '#title' => 'Dirección de Correspondencia del Solicitante',
                '#default_value' => $this->session->get('session_liquidacion.dir_correspondencia_afj', ''),
               ];
               $form['email'] = [
                '#type' => 'email',
               '#required' => TRUE,
                '#title' => 'Correo Electrónico',
                '#default_value' => $this->session->get('session_liquidacion.email_afj', ''),
               ];
               ...
               $form['tfijo'] = [
                '#type' => 'textfield',
                '#title' => 'Teléfono fijo',
                '#default_value' => $this->session->get('session_liquidacion.tfijo_afj', ''),
                ];
                ...
                $form['tmovil'] = [
                  ...
                  '#default_value' => $this->session->get('session_liquidacion.tmovil_afj', ''),
                 ];
   ...
   */
     public function validateForm(array &$form, FormStateInterface $form_state)
      {
         $cantidad_arboles = $form_state->getValue('cantidad_arboles_afj');;
         $cantidad_arboles_limit=26;

         if ($cantidad_arboles >= $cantidad_arboles_limit){
         $form_state->setErrorByName('cantidad_arboles_afj', $this->t('Por disposiciones legales si la solicitud supera 25 árboles, este trámite debe realizarse a través del sistema VITAL del MinAmbiente en : http://vital.minambiente.gov.co'));
         }
 ...
 $valor_liquidacion = $this->session->get('session_liquidacion.valorLiquidacion_afj','');
 $valor2 = $this->session->get('session_liquidacion.tarifa_afj', '');
//Construir factura
...
$codigo_liquidacion =$this->session->get('session_liquidacion.id_document_afj', '');
$barrio_liquidacion = $this->session->get('session_liquidacion.barrio_afj', '');
$direccion_predio_liquidacion = $this->session->get('session_liquidacion.dir_predio_afj', '');
$name_predio= $this->session->get('session_liquidacion.name_predio_afj', '');
$numero_arboles  = $this->session->get('session_liquidacion.cantidad_arboles_afj', '');
$especie_arboles  = $this->session->get('session_liquidacion.especie_afj', '');
$tipo_solicitante = "Persona Natural";
$id_contribuyente = $this->session->get('session_liquidacion.id_document_afj', '');
$name_contrib= $this->session->get('session_liquidacion.name_afj', '');

$dir_correspondecia_contrib = $this->session->get('session_liquidacion.dir_correspondencia_afj', '');

$email_cotrib = $this->session->get('session_liquidacion.email_afj', '');

$tfijo =  $this->session->get('session_liquidacion.tfijo_afj', '');
$tmovil =$this->session->get('session_liquidacion.tmovil_afj', '');
$estrato =  $this->session->get('session_liquidacion.estrato_afj', '');
$condicion = $this->session->get('session_liquidacion.condicion_afj', '');
...
">$ '.$valor_liquidacion.'</p>
      ...
      <tr>
        ...
        <div class="concepto">
        <p class="concepto">LIQUIDACIÓN DE EVALUACIÓN TECNICA PARA APROVECHAMIENTO FORESTAL,TALA PODA Y/O TRASLADO DE '.$numero_arboles.' ÁRBOLES, SEGÚN SOLICITUD #'.$sec.'</p>
        </div>
        </td>
      </tr>
...
*/
  ...
  public function fapiExampleMultistepFormNextValidate(array &$form, FormStateInterface $form_state) {
	...
	$this->setSessionValue('session_liquidacion.tarifa_afj',$valor2);
     ...
     $this->setSessionValue('session_liquidacion.valorLiquidacion_pdf',$valor_liquidacion);
	   ...
	   $this->setSessionValue('session_liquidacion.valorLiquidacion_afj', $valor_liquidacion);
           ...
           public function fapiExampleMultistepFormNextSubmit(array &$form, FormStateInterface $form_state)
           {
             //next1
              ...
              'name' => $form_state->getValue('name'),
               ...
               ->setRebuild(TRUE);
                   $this->setSessionValue('session_liquidacion.id_document_afj', $form_state->getValue('id_document'));
                   $this->setSessionValue('session_liquidacion.name_afj', $form_state->getValue('name'));
			       $this->setSessionValue('session_liquidacion.nombrelegal_afj', $form_state->getValue('nombrelegal'));
                   $this->setSessionValue('session_liquidacion.idlegal_afj', $form_state->getValue('idlegal'));

                   $this->setSessionValue('session_liquidacion.dir_correspondencia_afj', $form_state->getValue('dir_correspondencia'));

                   $this->setSessionValue('session_liquidacion.email_afj', $form_state->getValue('email'));
                   //$this->setSessionValue('session_liquidacion.estrato_afj', $form_state->getValue('first_estrato'));
                   $this->setSessionValue('session_liquidacion.tmovil_afj', $form_state->getValue('tmovil'));
                   $this->setSessionValue('session_liquidacion.tfijo_afj', $form_state->getValue('tfijo'));
             }
 ...
 /**
  ...
  public function fapiExampleMultistepFormNextSubmit2(array &$form, FormStateInterface $form_state) {
    ...
    $form_state
      ->set('page_values3', [
        // Keep only first step values to minimize stored data.
        'cantidad_arboles_afj' => $form_state->getValue('cantidad_arboles_afj'),
      ...
      ->setRebuild(TRUE);
      $this->setSessionValue('session_liquidacion.barrio_afj', $form_state->getValue('barrio'));
      $this->setSessionValue('session_liquidacion.dir_predio_afj', $form_state->getValue('dir_predio'));
      $this->setSessionValue('session_liquidacion.name_predio_afj', $form_state->getValue('name_predio'));
      $this->setSessionValue('session_liquidacion.cantidad_arboles_afj', $form_state->getValue('cantidad_arboles_afj'));
      $this->setSessionValue('session_liquidacion.especie_afj', $form_state->getValue('especie'));
  ...
  public function fapiExampleMultistepFormNextSubmit3(array &$form, FormStateInterface $form_state) {
    ...
    //next3
      ...
      // to 2, the AJAX-rendered form will still show page 3.
      ->setRebuild(TRUE);
      $this->setSessionValue('session_liquidacion.soportes1_afj', $form_state->getValue('soportes1'));

      $this->setSessionValue('session_liquidacion.soportes2_afj', $form_state->getValue('soportes2'));
      $this->setSessionValue('session_liquidacion.soportes3_afj', $form_state->getValue('soportes3'));
      $this->setSessionValue('session_liquidacion.soportes4_afj', $form_state->getValue('soportes4'));
  }
  /**
  ...
  public function fapiExamplePageTwo(array &$form, FormStateInterface $form_state) {
   ...
   '#required' => TRUE,
     ...
     '#default_value' => $this->session->get('session_liquidacion.barrio_afj', ''),
    ];
...
'#default_value' => $this->session->get('session_liquidacion.dir_predio_afj', ''),
];
...
'#default_value' => $this->session->get('session_liquidacion.name_predio_afj', ''),
];
$form['s1'] = [
'#type' => 'markup',
'#markup' => '<hr>',
...
$form['cantidad_arboles_afj'] = [
  ...
  '#default_value' => $this->session->get('session_liquidacion.cantidad_arboles_afj', ''),
  '#required' => TRUE,
  ];
...
'#default_value' => $this->session->get('session_liquidacion.especie_afj', ''),
  ...
  $form['list'] = [
    ...
    $form['soportes1'] = array(
      ...
      '#default_value' => $this->session->get('session_liquidacion.soportes1_afj', ''),
    );
    ...
    $form['soportes2'] = array(
      ...
      '#upload_validators' => $validators,
      '#upload_location' => 'public://my_files/privado',
      '#default_value' => $this->session->get('session_liquidacion.soportes2_afj', ''),
    );
    ...
    $form['soportes3'] = array(
      ...
      '#upload_validators' => $validators,
      '#upload_location' => 'public://my_files/privado',
      '#default_value' => $this->session->get('session_liquidacion.soportes3_afj', ''),
    );
    $form['soportes4'] = array(
      ...
      '#upload_validators' => $validators,
      '#upload_location' => 'public://my_files/privado',
      '#default_value' => $this->session->get('session_liquidacion.soportes4_afj', ''),
    );
  ...
  public function fapiExamplePageFour(array &$form, FormStateInterface $form_state) {
    ...
    // $form_state->set('page_num', 4);
     ...
     $id= $this->session->get('session_liquidacion.id_document_afj', '');
     $email=$this->session->get('session_liquidacion.email_afj', '');
     $Cantidad=$this->session->get('session_liquidacion.cantidad_arboles_afj', '');
     $valor2 = $this->session->get('session_liquidacion.tarifa_afj', '');
     $valor_liquidacion = $this->session->get('session_liquidacion.valorLiquidacion_afj','');
